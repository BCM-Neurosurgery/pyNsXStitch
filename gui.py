...
class StitcherGUI(object):
    ...
    def build_comment_table(self):
        ...
        col_widths = [0.1, 0.1, 0.3, 0.3, 0.1, 0.1]

        # Build the headers
        header_frame = tk.Frame(self.comment_frame)
        for col, width in zip(self.comment_df.columns, col_widths):
            tk.Label(header_frame, text=col, width=int(width * 100)).pack(side=tk.LEFT)

        start_stop_label = tk.Label(header_frame, text='Start/Stop', width=int(0.2 * 100))
        ...
        self.table_area = tk.Canvas(
            self.comment_frame,
            width=self.comment_frame.winfo_width(),
            height=self.comment_frame.winfo_height() - header_frame.winfo_height(),
            yscrollcommand=self.scroll.set
        )
        self.table_area.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
        self.table_area.bind('<Configure>', self.on_canvas_configure)
        ...
        # Create a frame inside the canvas to hold comment table content (so we can use grid)
        self.table_frame = tk.Frame(self.table_area)
        self.table_area.create_window((0, 0), window=self.table_frame, anchor=tk.NW)
        ...
        for i, row_data in enumerate(self.comment_df.iterrows()):
            ...
            for j, value in enumerate(row_data[1]):
                ...
                label = tk.Label(self.table_frame, text=text, width=int(col_widths[j] * 100), wraplength=int(col_widths[j] * 800), anchor=tk.NW, justify=tk.LEFT)
                label.grid(row=i, column=j, padx=5, pady=5, sticky=tk.W)
            ...
            start.grid(row=i, column=4, padx=5, pady=5)
            ...
            stop.grid(row=i, column=5, padx=5, pady=5)
        
        # Force tkinter to update layout and geometry of table_frame and its children before proceeding
        self.table_frame.update_idletasks()
        # Set scroll region to the bounding box of all items inside canvas
        self.table_area.config(scrollregion=self.table_area.bbox(tk.ALL))

    # Update width of table_frame when canvas is resized
    def on_canvas_configure(self, event):
        self.table_area.configure(scrollregion=self.table_area.bbox(tk.ALL))
        items = self.table_area.find_all()
        if items:
            self.table_area.itemconfig(items[0], width=event.width)
